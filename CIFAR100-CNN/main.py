...
from ds.runner import Runner, run_epoch, run_final_test
...
def main(cfg: CIFAR100Config):
    ...
    VAL_DATA, FINAL_TEST_DATA = random_split(
        TEST_DATA, [len(TEST_DATA)-val_size, val_size]
    )
    ...
    final_test_loader = create_device_dataloader(2*cfg.params.batch_size,
                                                 FINAL_TEST_DATA,
                                                 num_workers=cfg.workers, shuffle=False)
    ...
    train_runner = Runner(train_loader, model,
                          ...
                          grad_clip=cfg.params.grad_clip)
    ...
    # Make a Confusion Matrix for the small Test set
    with torch.no_grad():
        run_final_test(model, final_test_loader, tracker)
    tracker.flush()

    # Clear the GPU Memory Cache one last time
    if torch.device.type == 'cuda':
        torch.cuda.empty_cache()
